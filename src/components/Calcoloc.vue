<template>
  <div class="container-fluid">
      ...
      <v-col cols="12" style="background-color: #fcfcfc;border-radius: 10px;margin-left: 12px;margin-top: 40px;flex: auto;margin-bottom: 60px;">
        <div style="padding: 10px;padding-bottom: 0px;background-color: #E5E5E5;border-radius: 10px;overflow-x: scroll;">
          <table id="table" style="width: 100%;">
            <tr>
              <th style="width: 15%;padding-top: 3px;padding-bottom: 3px;background-color: transparent;color: black;font-size: 12px;">Denominazione</th>
              <th style="width: 15%;padding-top: 3px;padding-bottom: 3px;background-color: transparent;color: black;font-size: 12px;">Codice Regionale</th>
              <th style="width: 15%;text-align: center;padding-top: 3px;padding-bottom: 3px;background-color: transparent;color: black;font-size: 12px;">Codice Catalogo</th>
              <th style="width: 15%;padding-bottom: 3px;background-color: transparent;color: black;font-size: 12px;">Tipologia</th>
              <th style="width: 15%;padding-bottom: 3px;background-color: transparent;color: black;font-size: 12px;">Indice</th>
              <th style="width: 20%;padding-bottom: 3px;background-color: transparent;color: black;font-size: 12px;">Azioni</th>
            </tr>
            <tbody>
              <tr v-for="(healthCaresCupIndexe, index) in healthCaresCupIndexes" :key="healthCaresCupIndexe" :class="{'even-row': ((index+1)%2==0), 'odd-row': ((index+1)%2!=0)}">
                <td>{{healthCaresCupIndexe.description}}</td>
                <td>{{healthCaresCupIndexe.regional_code}}</td>
                <td>{{healthCaresCupIndexe.catalog_code}}</td>
                <td>{{healthCaresCupIndexe.type}}</td>
                <td>{{healthCaresCupIndexe.idx}}</td>
                <td>
                  <button @click="editPopup(healthCaresCupIndexe)" class="select-btn selectedBtn" style="font-size: 12px;padding: 5px 10px 5px 10px;"><i class="fa fa-pencil"></i> Modifica</button>
                  <button @click="deletePopup(healthCaresCupIndexe)" class="select-btn selectedBtn" style="font-size: 12px;padding: 5px 10px 5px 10px;background-color: #ff647c;"><i class="fa fa-trash"></i> Elimina</button>
                </td>
              </tr>
            </tbody>
          </table>
        </div>
        <div style="margin-top: 10px;text-align: center;" v-if="healthCaresCupIndexes.length>0">
    ...
    <modal-vue @on-close="closePopup" :name="'calcoloc'" :headerOptions="{title: '  '}" noFooter style="z-index: 100;" class="dialog-width dialog-width-31">
      <div>
            ...
            <tbody>
              <tr style="border: none;">
                <th colspan="2">
                  ...
                  <div v-if="$v.indice.$dirty && $v.indice.$invalid" class="v-messages theme--light error--text" role="alert">
                    <div class="v-messages__wrapper">
                      <div v-if="$v.indice.$dirty && $v.indice.$invalid" class="v-messages__message message-box">Indice</div>
                    </div>
                  </div>
                </th>
              </tr>
              ...
              <tr style="border: none;">
                <td class="form-label">Indice</td>
                <td class="form-input-td layout_placeholder">
                  <select v-model="indice" style="padding-top: 2px;min-width: 167px;border: 1px solid #cfc9d6;border-radius: 5px;" :class="{'error-border': $v.indice.$dirty && $v.indice.$invalid}" >
                    <option :key="null" :value="null">-----------------</option>
                    <option :key="'D13C'" :value="'D10Z'">D10Z</option>
                    <option :key="'D11Z'" :value="'D11Z'">D11Z</option>
                    <option :key="'D13C'" :value="'D13C'">D13C</option>
                  </select>
                </td>
              </tr>
              <tr style="border: none;">
    ...
    <modal-vue @on-close="closeDeletePopup" :name="'deleteCalcoloc'" :headerOptions="{title: '  '}" noFooter style="z-index: 100;" class="dialog-width">
      <div>
...
import Paginate from 'vuejs-paginate';
  export default {
    components: {Paginate},
    mounted() {
      this.getHealthCaresCupIndexesData();
    },
    data () {
      return {
        id: null,
        healthCaresCupIndexes: [],
        denominazione: null,
    ...
    methods: {
      getHealthCaresCupIndexesData () {
        axios
        .get(
            this.$config.API_URL+"getHealthCaresCupIndexes?page="+this.pageNumber,{
            headers: {
        ...
        .then(response => {
          this.healthCaresCupIndexes = response.data.healthCaresCupIndexes;
      ...
      pushHealthCaresCupIndexes () {
        axios
        .post(
            this.$config.API_URL+"pushHealthCaresCupIndexes",
            {
              "id": this.id,
      ...
      deleteHealthCaresCupIndexes () {
        axios
        .delete(
            this.$config.API_URL+"deleteHealthCaresCupIndexes", {
            headers: {
              Authorization: 'Bearer' + localStorage.getItem('token'),
      ...
      openPopup() {
        ...
        this.indice = null;
        this.$v.$reset();
        this.$vm2.open('calcoloc');
      },
      closePopup() {
        this.$vm2.close('calcoloc');
      },
      editPopup(healthCaresCupIndexe) {
        this.id = healthCaresCupIndexe.id;
        this.denominazione = healthCaresCupIndexe.description;
        this.codiceRegionale = healthCaresCupIndexe.regional_code;
        this.codiceCatalogo = healthCaresCupIndexe.catalog_code;
        this.tipologia = healthCaresCupIndexe.type;
        this.indice = healthCaresCupIndexe.idx;
        this.$v.$reset();
        this.$vm2.open('calcoloc');
      },
      deletePopup(healthCaresCupIndexe) {
        this.id = healthCaresCupIndexe.id;
        this.$vm2.open('deleteCalcoloc');
      },
      closeDeletePopup() {
        this.$vm2.close('deleteCalcoloc');
      },
      save() {
        this.$v.$touch();
        if(!this.$v.$invalid){
          this.pushHealthCaresCupIndexes();
        }
      },
      deleteSelected() {
        this.deleteHealthCaresCupIndexes();
      },
